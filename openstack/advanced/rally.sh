...
openstack project create rally-project
for i in 1 2 3
do
openstack user create --project rally-project --password unix123 test${i}
done
...
sed -i "s#\$OS_AUTH_URL#${OS_AUTH_URL}#g" /root/existing-with-predefined-users.json
...
rally  deployment create --filename /root/existing-with-predefined-users.json --name cloud-rally
rally task start --abort-on-sla-failure /root/rally-tasks.yaml
rally task report --out=/root/cloud-rally-report.html
